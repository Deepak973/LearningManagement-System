                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                <div id="simpletable_filter" class="dataTables_filter">
                                    <a href="{{ route('authusers.create') }}" class="btn btn-primary">
                        ...
                        @if($authusers->count() > 0)
                            <table id="simpletable" class="table table-striped table-bordered nowrap dataTable" role="grid" aria-describedby="simpletable_info">
                                <thead>
                                    ...
                                    <th class="sorting" width="5%" tabindex="0" aria-controls="simpletable">User_type</th>
                                    <th class="sorting" width="5%" tabindex="0" aria-controls="simpletable">Batch_id</th>
                                    <th class="sorting" width="5%" tabindex="0" aria-controls="simpletable">Action</th>
                                </thead>
                                <tbody>
                                    @foreach ($authusers as $index => $user)
                                    <tr>
                                        <td class="align-top">{{ $index + 1 }}</td>
                                        <td class="align-top">
                                            {{ $user->email }}
                                        </td>
                                        <td class="align-top">{{ $user->user_type}}</td>
                                        <td class="align-top">{{ $user->batch_id }}</td>
                                        <td class="align-top">
                                            <form action="{{ route('authusers.destroy', [ 'authuser' => $user->id ]) }}" method="POST">
                                                @csrf
                                                @method("DELETE")
                                                <a href="{{ route('authusers.show', $user->id) }}">
                                                    <i class="ik ik-eye f-16 mr-15 text-green"></i>
                                                </a>
                                                <a href="{{ route('authusers.edit', $user->id) }}">
