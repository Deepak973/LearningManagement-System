        ...
        <div class="nav-container">
            <nav id="main-menu-navigation" class="navigation-main">
                <div class="nav-item {{ ($segment2 == 'dashboard') ? 'active' : '' }} ">
                    <a href="{{route('user_dashboard')}}"><i class="ik ik-bar-chart-2"></i><span>{{ __('Dashboard')}}</span></a>
                </div>
                <div class="nav-item {{ ($segment2 == 'sports') ? 'active' : '' }}">
                    <a href="{{ route('showcourse') }}"><i class="ik ik-book"></i><span>{{ __('Courses')}}</span></a>
                </div>
                <div class="nav-item {{ ($segment2 == 'bannerimages') ? 'active' : '' }} ">
                    <a href="{{ route('show_user_trainings') }}"><i class="ik ik-calendar"></i><span>{{ __('Trainings')}}</span></a>
                </div>
                <div class="nav-item {{ ($segment2 == 'users') ? 'active' : '' }}">
                    <a href="{{ route('show_trainee_attendance')}}"><i class="ik ik-clipboard"></i><span>{{ __('Attendance')}}</span></a>
                </div>
                <div class="nav-item {{ ($segment2 == 'contactus') ? 'active' : '' }}">
                    <a href="{{ route('show_trainee_assessments')}}"><i class="ik ik-globe"></i><span>{{ __('Assessments')}}</span></a>
                </div>
                
                <div class="nav-item {{ ($segment2 == 'amenities') ? 'active' : '' }}">
                    <a href="{{ route('show_trainee_marks') }}"><i class="ik ik-send"></i><span>{{ __('Assessments Result')}}</span></a>
                </div>
